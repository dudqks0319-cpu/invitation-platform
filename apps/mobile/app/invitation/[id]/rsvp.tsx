...
export default function InvitationRsvpScreen() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (!mounted) return;
        setError(caught instanceof Error ? caught.message : "RSVP 정보를 불러오지 못했습니다.");
      }
  ...
  return (
    <Screen subtitle="참석/불참, 동행 인원, 최근 응답을 운영자 시점에서 봅니다." title="RSVP 관리">
      ...
      {error ? <ErrorView description={error} title="RSVP 불러오기 실패" /> : null}
      {message ? (
      ...
      <Card eyebrow="대상 초대장" title={draft?.payload.title || "초대장"}>
        ...
        <View style={{ marginTop: 12 }}>
          <Button accessibilityLabel="RSVP 목록 새로고침" onPress={() => void refresh()} variant="outline">
            {refreshing ? "새로고침 중..." : "RSVP 새로고침"}
          </Button>
        </View>
      </Card>
      {entries.length === 0 ? (
        <EmptyState body="아직 수신된 RSVP가 없습니다." title="응답이 들어오면 여기에 표시됩니다" />
      ) : (
