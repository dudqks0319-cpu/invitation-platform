...
async function withAuthTimeout<T>(promise: PromiseLike<T>, message: string): Promise<T> {
...
export function useAuth() {
  ...
  useEffect(() => {
    ...
    void withAuthTimeout(supabase.auth.getSession(), "로그인 상태 확인 시간이 초과되었습니다.")
      .then(({ data }) => {
        if (!mounted) return;
      ...
      ensureAnonymousSession: async () => {
        ...
        try {
          ...
          return await withAuthTimeout(
            ...
            "게스트 세션 시작 시간이 초과되었습니다."
          );
        } catch (caught) {
          return {
            data: { session: null, user: null },
            error: caught instanceof Error ? caught : new Error("게스트 세션을 시작하지 못했습니다.")
          };
        }
      },
