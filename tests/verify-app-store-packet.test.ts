...
describe("App Store packet verifier", () => {
  it("accepts selected clean source Build 69 while keeping its artifact unbound", () => {
    ...
    expect(result.stdout).toContain("- Latest Source Identity: 1.0.3 (69) / com.invitehub.app / source 902eefca50f6b305d429e5665d295c5e6db9e870");
    expect(result.stdout).toContain("highest production Build 68 and Build 69 count 0");
    expect(result.stdout).toContain("Candidate Verdict: HOLD");
  });
  ...
  it("rejects an exact-build-installed claim without artifact provenance", () => {
    ...
    expect(result.status).toBe(1);
    expect(result.stderr).toContain(
      "release-ledger.yaml: expected to include phase: candidate_selected_clean_verification_blocked"
    );
  });
