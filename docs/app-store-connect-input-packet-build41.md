# InviteHub App Store Connect Input Packet - Build 41

Date: 2026-05-03

Use this packet when entering App Store Connect fields for build `1.0.0 (41)`.
Do not press final review submission until the user explicitly confirms that
action.
...
Select build `1.0.0 (41)` for the App Store version only after the iPhone
...
open-confirmation prompt, dev overlay, or simulator window chrome. The build 41
code change only affects release routing; these screenshots remain valid for
the current App Store metadata packet.
...
```bash
node scripts/record-app-store-evidence.mjs --key realIphoneTestFlightInstallLaunchPassed --evidence "<real iPhone installed TestFlight build 1.0.0 (41) and launched home/template/builder/preview without the crash dialog>" --artifact "output/testflight-device-watch/<timestamp>/evidence/<timestamp>"
...
node scripts/record-app-store-evidence.mjs --key build41SelectedForVersion --evidence "<build 1.0.0 (41) selected for version 1.0.0>" --artifact "https://appstoreconnect.apple.com/apps/6763630299/distribution"
```
