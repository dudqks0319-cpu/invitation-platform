...
### App Name
Preferred: 오삼오삼 - 모바일 초대장

Fallback if the full name is unavailable: 오삼오삼
...
### Description
오삼오삼은 결혼식과 각종 행사 초대장을 모바일에서 손쉽게 만들고 공유할 수 있는 앱입니다.
...
오삼오삼은 “디자인을 먼저 고르고, 필요한 정보만 채워 공유한다”는 방향으로 초대장 경험을 만들고 있습니다.
...
Use this version only for a free-only submission where paid publish remains
hidden:
...
Paid IAP submission version, after RevenueCat and App Store Connect version IAP
attachment are complete:

- 로그인 없이도 앱 내에서 초대장 초안을 만들고 미리보기할 수 있습니다.
- 계정이 있는 경우 원격 저장, 발행권 구매, 공개 링크 발행, RSVP/방명록 관리가 가능합니다.
- 발행권은 Apple 인앱결제 상품 `com.invitehub.publish.credit`로만 구매합니다.
- 앱 내부에 웹 결제, PG 결제, 계좌이체 문의, 카카오페이 결제 유도 문구는 없습니다.
- 발행권 1개는 초대장 1개 최종 발행, 공유 링크, RSVP, 방명록, 지도/계좌 정보 표시를 포함합니다.
- 발행권 구매 화면에는 구매 복원 버튼이 있습니다.
- 결제 성공은 RevenueCat webhook을 통해 서버 크레딧으로 적립한 뒤 발행 시 차감합니다.
- 환불/취소 이벤트는 미사용 발행권 회수로 처리합니다.
- 방명록은 작성 직후 공개되지 않고 호스트가 대시보드에서 승인하거나 숨길 수 있습니다.
- 지도는 앱 내부 지도 타일을 직접 표시하지 않고, 장소명/주소를 기준으로 카카오맵과 네이버지도 검색 링크를 엽니다.
...
- Product ID: `com.invitehub.publish.credit`
- Type: Consumable
- Display Name: 사진 포함 발행권
- Price Target: 3,300원
- Server Env: `STORE_PUBLISH_PRODUCT_IDS_IOS=com.invitehub.publish.credit`
- Mobile Env: `EXPO_PUBLIC_IAP_PRODUCT_IDS_IOS=com.invitehub.publish.credit`
- RevenueCat Package: `publish_credit_1`
- RevenueCat Webhook: `/api/payments/revenuecat/webhook`
...
- Identifiers: 사용자 ID, 초대장 ID. 앱 기능 제공 및 사용자 콘텐츠 소유권 관리 목적.
...
- Identifiers: 결제 트랜잭션 ID. 서버 크레딧 적립/차감, 환불/취소 회수, 부정 사용 방지 목적.
...
### Full Description
오삼오삼은 결혼식과 각종 행사 초대장을 모바일에서 쉽게 만들고, 링크로 공유하고, 응답과 방명록까지 함께 관리할 수 있는 앱입니다.
