...
Last updated: 2026-05-19 22:54 KST
...
| Current candidate | Build `1.0.1 (48)` built by EAS; submission is queued for App Store Connect upload. |
...
| App Store Connect / TestFlight state | Build 48 finished in EAS; EAS submission is `IN_QUEUE`, so Apple upload/processing/TestFlight group assignment are not yet verified |
...
embedded. However, EAS submission is still queued, so Apple processing,
TestFlight internal-group assignment, and a real iPhone launch/free-publish
smoke test for build 48 are still required.
...
1. Wait for Build 48 EAS submission to finish, then wait for App Store Connect
   processing.
...
- EAS status recheck returned build `FINISHED`, submission `IN_QUEUE`, and
  submission `error: null`.
