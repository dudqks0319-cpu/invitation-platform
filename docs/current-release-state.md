...
Last updated: 2026-07-26 08:02 KST
...
| Release source commit | `51272629c70d50be99a75b82e8d028cc996be175` |
...
| Template copy | 카드의 흰색 문구 칩을 제거하고 실제 미리보기 예시의 이름·날짜·장소를 이미지 안전 여백에 직접 표시; 작은 카드는 같은 크기로 최대 3줄을 사용해 말줄임표 제거 |
...
| Simulator result | Release build/install/launch passed on iPhone 17 Pro, iOS 26.2; home cards, recent list, and four corrected previews individually verified with one image, complete copy, transparent safe zones, and no subject overlap |
| Visual verdict | `97/100`, pass; complete compact titles, consistent typography, no white text chips, and no copy overlap with people/illustrations |
...
| Superseded builds | Builds 62, 63, and 64 must not be selected for review; Build 64 predates the final single-preview, enlarged-copy, curated-artwork, and compact-title fixes |
...
| TestFlight result | Build 64 is `제출 준비 완료` in the `Team (Expo)` internal group, but it is superseded by local source `51272629c70d50be99a75b82e8d028cc996be175` |
...
`51272629c70d50be99a75b82e8d028cc996be175`. Its Release simulator build
succeeded. The home cards, recent list, and the corrected housewarming, baby,
graduation, and business previews were exercised individually. Copy is complete
and consistently sized, uses transparent safe zones, and does not overlap the
...
single-preview, enlarged-copy, and curated-artwork fixes, so it is superseded and must not be
...
1. Produce and upload a new iOS build greater than Build 64 from exact source `51272629c70d50be99a75b82e8d028cc996be175`.
