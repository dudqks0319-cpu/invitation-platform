...
| TestFlight device evidence harness | `scripts/collect-testflight-device-evidence.sh`, `scripts/await-testflight-device.sh` | Added focused real-device evidence collection and wait harnesses for device state, lock state, TestFlight app metadata, `com.invitehub.app` app metadata, InviteHub process state, optional TestFlight launch, and optional user-observed InviteHub launch. Latest evidence bundle `output/testflight-device-watch/20260503-143921/evidence/20260503-143922` reached the paired iPhone, found installed build `39`, and launch was denied because the phone was locked. | Ready, blocked on unlocked iPhone with build 40 installed |
...
- `SKIP_IOS_RELEASE_BUILD=1 zsh scripts/invitehub-release-gate.sh`:
  ...
  focused 9-file mobile/API test suite with 34 tests. The 60-check App Store
...
- Fast release gate recheck on 2026-05-03 13:24 KST:
  ...
  tests. The new 60-check App Store packet verifier is pinned to build 40.
