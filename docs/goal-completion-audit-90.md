...
| TestFlight crash triage | `docs/testflight-crash-triage-2026-05-03.md`, ASC TestFlight iOS build table, native config tests | Read-only ASC audit shows build 40 has invite `1` but installs/sessions/crashes `-`; build 39 has install `1` and crash `1`; build 38 has install `1` and crashes `3`. Production/dev bundle ids are separated, so the leading hypothesis is an old build or wrong TestFlight entry rather than two apps colliding. | Blocked on iPhone build 40 install proof |
| TestFlight device evidence harness | `scripts/collect-testflight-device-evidence.sh`, `scripts/await-testflight-device.sh` | Added focused real-device evidence collection and wait harnesses for device state, lock state, `com.invitehub.app` app metadata, InviteHub process state, and optional user-observed launch. Dry runs on 2026-05-03 wrote ignored evidence/watch bundles and reproduced the current CoreDevice unavailable blocker. | Ready, blocked on available iPhone |
...
- `SKIP_IOS_RELEASE_BUILD=1 zsh scripts/invitehub-release-gate.sh`:
  ...
  focused 9-file mobile/API test suite with 34 tests. The 54-check App Store
...
- Fast release gate recheck on 2026-05-03 13:24 KST:
  ...
  tests. The new 54-check App Store packet verifier is pinned to build 40.
