...
| TestFlight device evidence harness | `scripts/collect-testflight-device-evidence.sh` | Added a focused real-device evidence collector for device state, lock state, `com.invitehub.app` app metadata, InviteHub process state, and optional user-observed launch. A dry run on 2026-05-03 13:46 KST wrote an ignored evidence bundle and reproduced the current CoreDevice unavailable blocker. | Ready, blocked on available iPhone |
